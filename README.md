...
|---|---|---|
| **Codebase** | Total source lines | **699** |
| | Backend (`server.js`) | **136 lines** |
| | Frontend (`src/`) | **563 lines** |
| | Source files | **7** |
| | API endpoints | **3** |
| | WebSocket events | **5** |
| **Dependencies** | Runtime packages | **17** |
| | Dev packages | **1** |
| **Bundle** | JS (gzipped) | **280.0 KB** |
| | CSS (gzipped) | **1.4 KB** |
| | Total (gzipped) | **281.4 KB** |
| **Runtime** | `POST /api/login` | **72ms avg** (bcrypt verify, 3 runs) |
| | `POST /api/execute` | **7ms** (simple) / **10ms** (1M iterations) |
| | WebSocket connect | **10ms** |
| | Yjs sync (join → first state) | **2ms** |
| **Security** | bcrypt rounds | **10** (~100ms per hash) |
| | JWT expiry | **7 days** |
| | VM execution timeout | **5 seconds** |
